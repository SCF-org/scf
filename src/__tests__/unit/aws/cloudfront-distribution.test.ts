...
describe('CloudFront Distribution Management', () => {
  ...
  describe('distributionExists', () => {
    ...
    it('should throw error for other failures', async () => {
      ...
      await expect(distributionExists(client, 'E1234567890ABC')).rejects.toThrow('Access denied');
    });
  });
  ...
  describe('getDistribution', () => {
    ...
    it('should throw error for other failures', async () => {
      cfMock.on(GetDistributionCommand).rejects(new Error('Service unavailable'));
  ...
  describe('createDistribution', () => {
    ...
    it('should create distribution for non-us-east-1 region', async () => {
      ...
      const calls = cfMock.commandCalls(CreateDistributionCommand);
      expect(calls[0].args[0].input.DistributionConfig?.Origins?.Items?.[0]?.DomainName).toBe(
        'my-bucket.s3-website.ap-northeast-2.amazonaws.com'
      );
    });
    ...
    it('should create distribution with multiple aliases', async () => {
      ...
      expect(config?.Aliases?.Items).toEqual(['example.com', 'www.example.com']);
      expect(config?.Aliases?.Quantity).toBe(2);
    });

    it('should create distribution with custom cache settings', async () => {
      cfMock.on(CreateDistributionCommand).resolves({
        Distribution: mockDistribution,
      });
      ...
      expect(config?.DefaultCacheBehavior?.DefaultTTL).toBe(3600);
      expect(config?.DefaultCacheBehavior?.MaxTTL).toBe(86400);
      expect(config?.DefaultCacheBehavior?.MinTTL).toBe(60);
    ...
    it('should throw error when creation fails', async () => {
      cfMock.on(CreateDistributionCommand).rejects(new Error('Invalid configuration'));
      ...
      await expect(createDistribution(client, options)).rejects.toThrow('Invalid configuration');
    });
  ...
  describe('updateDistribution', () => {
    ...
    it('should update distribution price class', async () => {
      ...
      expect(calls).toHaveLength(1);
      expect(calls[0].args[0].input.DistributionConfig?.PriceClass).toBe('PriceClass_All');
    ...
    it('should update cache TTL settings', async () => {
      ...
      const calls = cfMock.commandCalls(UpdateDistributionCommand);
      const behavior = calls[0].args[0].input.DistributionConfig?.DefaultCacheBehavior;

      expect(behavior?.DefaultTTL).toBe(3600);
      expect(behavior?.MaxTTL).toBe(86400);
      expect(behavior?.MinTTL).toBe(60);
    });
    ...
    it('should update IPv6 setting', async () => {
      ...
      const calls = cfMock.commandCalls(UpdateDistributionCommand);
      expect(calls[0].args[0].input.DistributionConfig?.IsIPV6Enabled).toBe(false);
    });
    ...
    it('should update custom domain and certificate', async () => {
      ...
      await updateDistribution(client, 'E1234567890ABC', {
        customDomain: {
          ...
          certificateArn: 'arn:aws:acm:us-east-1:123456789012:certificate/new123',
        },
      });
    ...
    it('should throw error if DefaultCacheBehavior is missing', async () => {
      ...
      ).rejects.toThrow('Distribution configuration missing DefaultCacheBehavior');
    });
